...
function MarkdownImage({ src, alt }: { src?: string; alt?: string }): React.JSX.Element | null {
  if (!src) return null;
  // SVG files must not go through the Next.js image optimizer (/_next/image) because the
  // optimizer returns HTTP 400 for SVGs unless dangerouslyAllowSVG is set. Use the raw
  // path directly; local /images/*.svg static files serve fine without a proxy.
  // Non-SVG images still need the proxy to bypass api.mudbase.dev's CORP header.
  const resolvedSrc = src.toLowerCase().endsWith(".svg") ? src : proxiedImageSrc(src);
